...
parser.add_argument('-td', '--target_directory', default='/Users/gballardin/Downloads/', help='target directory where to save files', type=str, required=False)
parser.add_argument('-trs', '--train_share', default=.8, type=float, help='share of train images')
...
while i < len(search_keyword):
	...
	while (k < limit):
		try:
			...
			if random.uniform(0, 1) < train_share:
				dir_name=train_dir_name
			else:
				dir_name=vali_dir_name
			if '?' in image_name:
				image_name = image_name[:image_name.find('?')]
			if ".jpg" in image_name or ".png" in image_name or ".jpeg" in image_name or ".svg" in image_name:
				_, extension = os.path.splitext('test.jpg')
				output_file = open(dir_name + "/" + str(k + 1) + "." + search_term + extension, 'wb')
			else:
				output_file = open(dir_name + "/" + str(k + 1) + "." + search_term + ".jpg", 'wb')
